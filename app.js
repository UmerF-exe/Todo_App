// app.js (module) - Updated for per-user Firestore structure

import { db, collection, addDoc, doc, deleteDoc, updateDoc, onSnapshot, auth, onAuthStateChanged } from './firebase.js';
...
// Firestore ref (per-user, set later when logged in)
let todosCol;
...
if (user) {
// Point to this user’s tasks subcollection
todosCol = collection(db, "todos", user.uid, "tasks");
...
if (!t) return;
await updateDoc(doc(todosCol, id), { completed: !t.completed });
} catch (err) {
...
if(!todosCol) return;
try {
await updateDoc(doc(todosCol, id), { text: newText });
} catch (err) {
...
updateDoc(doc(todosCol, t.id), { order: newOrder }).catch(err => console.error('order update err', err));
}
});
}
